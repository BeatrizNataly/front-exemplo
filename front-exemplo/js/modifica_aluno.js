...
function updateAlunosList() {
    ...
    self.alunos.forEach(aluno => {
        alunoEntries += `<tr><td>${aluno.first_name}</td><td>${aluno.last_name}</td><td>${aluno.email}</td><td>${aluno.phone}</td><td>${aluno.curso}</td><td><button class="btn" onClick="editAluno(${aluno.id})">Editar</button></td><td><button class="btn" onClick="apagaAluno(${aluno.id})">Apagar</button></td></tr>`
    });
